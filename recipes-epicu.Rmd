...
ingredmat <- as.matrix(t(ingredinmattrim)) +0
  

ingredfact <- factanal(ingredmat, 2, rotation="varimax")

ingredfact.lo <- as.data.frame(ingredfact$loadings[,1:2])
ingredfact.lo$clust <- colnames(ingreddf)
ingredfact.lo$size <- as.numeric(km.out.main$size)

ingredfact.lo <- left_join(ingredfact.lo, ingreding, by="clust")
ingredfact.lo$ingredsnum <- paste(ingredfact.lo$clust, "\n", ingredfact.lo$ingreds)

ggplot(ingredfact.lo, aes(x= Factor1, y=Factor2, fill=ingredsnum))+
  geom_point(shape=21, aes(size=size), alpha=0.6)+
  theme_void()+
  geom_text(size=5, aes(label=clust, colour=ingredsnum), nudge_x=-0.05)+
  geom_text(aes(label=ingredscol), alpha=0.6)+
